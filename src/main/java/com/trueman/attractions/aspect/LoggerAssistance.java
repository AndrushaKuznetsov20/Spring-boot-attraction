...
/**
 * Прокси класс, который перехватывает запросы для логирования бизнес-логики управления услугами.
 */
@Aspect
@Component
public class LoggerAssistance {
    private final Logger logger = LoggerFactory.getLogger(LoggerAssistance.class);

    /**
     * Действия, которые будут выполнены до вызова метода.
     */
    @Before("execution(* com.trueman.attractions.services.AssistanceService.*(..))")
     ...
     */
    @After("execution(* com.trueman.attractions.services.AssistanceService.*(..))")
     ...
     */
    @AfterThrowing(pointcut = "execution(* com.trueman.attractions.services.AssistanceService.*(..))", throwing = "exception")
